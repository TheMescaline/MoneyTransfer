...
public class TransferServiceImpl implements TransferService {
    ...
    public boolean doTransfer(TransferInfoPacket packet) {
        log.info(MessageFormat.format("Transferring {0} from account ID {1} to account ID {2}", packet.getFromAccount(), packet.getToAccount(), packet.getAmount()));
        return accountDAO.doTransfer(packet.getFromAccount(), packet.getToAccount(), packet.getAmount());
    }
}
